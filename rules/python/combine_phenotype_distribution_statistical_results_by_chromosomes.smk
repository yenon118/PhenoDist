rule combine_phenotype_distribution_statistical_results_by_chromosomes:
    input:
    ...
    output:
        out_file=os.path.join(os.path.abspath(output_folder),'combine_phenotype_distribution_statistical_results_by_chromosomes','{project_name}_{{chromosome}}.txt'.format(project_name=project_name))
    log:
        os.path.join(os.path.abspath(output_folder),'combine_phenotype_distribution_statistical_results_by_chromosomes_log','{project_name}_{{chromosome}}.log'.format(project_name=project_name))
    threads: threads
    resources:
        memory=memory
    shell:
